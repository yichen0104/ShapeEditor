...
const SVGLayer = () => {
  ...
  const handleMouseDown = (e) => {
    if (currMode !== "select") {
      ...
      if (e.target.nodeName === "svg") {
        ...
        moveAndUpdateShape({
          initCoords: shapesMap[shapes.filter((shapeId) => shapeId === targetId)[0]].initCoords,
          finalCoords: shapesMap[shapes.filter((shapeId) => shapeId === targetId)[0]].finalCoords,
        });
      }
    }
  };
  ...
  const handleMouseUp = (e) => {
    if (currMode !== "select") {
      ...
      console.log("create move command object");
      /*moveAndUpdateShape(
        {
          initCoords: initPoint,
          finalCoords: currPoint,
  ...
  useEffect(() => {
    ...
    //window.addEventListener("keydown", ctrlZDownHandler, true);
